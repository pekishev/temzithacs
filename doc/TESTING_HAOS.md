...
- `sensor.temzit_heat_pump_integration_potreblenie` - Потребление
- `sensor.temzit_heat_pump_integration_moshchnost_nagreva` - Мощность нагрева
- `sensor.temzit_heat_pump_integration_cop` - COP (коэффициент производительности)
- `sensor.temzit_heat_pump_integration_protok` - Проток
- `sensor.temzit_heat_pump_integration_kompressor_1` - Компрессор 1 (Hz)
- `sensor.temzit_heat_pump_integration_oshibka` - Ошибка
- `sensor.temzit_heat_pump_integration_sostoyanie` - Состояние (скрыт)
- `sensor.temzit_heat_pump_integration_raspisanie` - Расписание (скрыт)

**Климат:**
- `climate.temzit_heat_pump_integration_obogrev_doma` - Обогрев дома (с управлением температурой)

**Водонагреватели:**
- `water_heater.temzit_heat_pump_integration_goriachaia_voda` - Горячая вода (с управлением температурой)

### 3. Проверка обновления данных

1. Перейдите в **Настройки** → **Устройства и службы**
2. Найдите ваше устройство "Temzit heat pump"
...
Интеграция поддерживает управление температурой через протокол настроек:
- **Обогрев дома**: Установка целевой температуры отопления (0-50°C)
- **Горячая вода**: Установка целевой температуры ГВС (0-60°C)

**Как проверить:**
1. Откройте климат-устройство "Обогрев дома" или водонагреватель "Горячая вода"
2. Измените целевую температуру через интерфейс
3. Проверьте, что изменения применяются на устройстве
4. Убедитесь, что состояние обновляется после изменения (в течение 30 секунд)

**Примечание:** Интеграция получает текущие настройки, изменяет только нужный параметр и отправляет все настройки обратно, сохраняя остальные параметры устройства.
